...
def test_row_count(data: pd.DataFrame) -> None:
    """
    Tests the number of rows in the data is between 15,000 and 1,000,000

    Args:
        data: The dataset to be tested
    
    Raises:
        AssertionError: If the number of records is <= 15,000 or >= 1,000,000
    """
    assert 15000 < data.shape[0] < 1000000

def test_price_range(data: pd.DataFrame, min_price: float, max_price: float) -> None:
    """
    Tests if all values in the 'price' column are between min_price and max_price

    Args:
        data: The dataset to be tested
        min_price: The minimum price threshold
        max_price: The maximum price threshold
    
    Raises:
        AssertionError: If any of the values in data['price'] are less than min_price or greater than max_price
    """
